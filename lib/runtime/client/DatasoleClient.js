...
class DatasoleClient extends EventEmitter {
  ...
  connect() {
    ...
    try {
      this._connectStartTimeMs = Date.now();
  ...
  onConnected() {
    const connectTimeMs = Date.now() - this._connectStartTimeMs;
    console.debug(
      `Connected to WebSocket endpoint ${this.wsUrl} in ${connectTimeMs} ms`
    );
