...
class WebSocketServer extends BaseServer {
  ...
  sendStateRefresh(clientId) {
    ...
    this.sendOne(clientId, makeMessagePacket(applyOp));
  }
  ...
  async broadcast(message) {
    ...
    // Encode the message once and send it to all clients in concurrent batches.
    if (!message) return Promise.resolve();
    const msgPacketStr = makeMessagePacket(message);
    const broadcasters = map(this._connections, (_, clientId) =>
      callbackify(async () => await this.sendOne(clientId, msgPacketStr))
    );

    await promisify(parallelLimit)(broadcasters, BROADCAST_CONCURRENCY);
    return Promise.resolve();
  }

  /**
   * Sends a JSON message to a single connected client if it is still connected.
   *
   * @param {string} clientId Client ID to send message to.
   * @param {string} message JSON message.
   */
  sendOne(clientId, message) {
    assert(isString(message), "message passed to sendOne() must be a string.");
  ...
  onClose(clientId) {
    ...
    this._context.log.debug(`Client ${clientId} dropped.`);
  }
