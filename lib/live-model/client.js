...
export class LiveModelClient extends EventEmitter {
  ...
  onMessage(msg) {
    ...
    // Parse Websocket message string.
    const msgStr = msg.data;
    let msgPayload;
    ...
    stats.avgMsgSizeBytes = Math.round(
      (stats.avgMsgSizeBytes * stats.msgsReceived + msgStr.length) /
        (stats.msgsReceived + 1),
      0
    );
    stats.avgMsgDecodeTimeMs = Math.round(
      (stats.avgMsgDecodeTimeMs * stats.msgsReceived + decodeTimeMs) /
        (stats.msgsReceived + 1),
      0
    );
    stats.msgsReceived++;
