...
const CMDLINE_ARGS = {
  description: "Run a production webserver (serves minimized Webpack bundle).",
  options: [
...
async function main(args) {
  ...
  if (config.cli.frontend) {
    await startWebserver(config, liveModelServer, { production: true });
  }
  if (config.cli.backend) {
    await startBackend(config, liveModelServer, { production: true });
  }
}
