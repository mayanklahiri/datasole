...
class WebSocketServer extends BaseServer {
  ...
  async authorize(wsAuthRequest) {
    ...
    if (this._wsAuthRequests[clientId]) {
      ...
      log.info(`Allowing WS connection for client "${clientId}"`);
      wsServer.handleUpgrade(request, socket, head, ws => {
        wsServer.emit("connection", ws, request);
      });
    } else {
      log.warn(`Cannot authorize unknown WS request for client "${clientId}"`);
    }
  }
  ...
  async refuseAuthRequest(wsAuthRequest, error) {
    ...
    if (this._wsAuthRequests[clientId]) {
      const [request, socket, head, timeout] = this._wsAuthRequests[clientId];
      log.info(
        `Destroying unauthorized WS connection for client "${clientId}"`
      );
      if (socket.writable) {
