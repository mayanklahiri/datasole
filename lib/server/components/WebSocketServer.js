...
class WebSocketServer extends BaseServer {
  ...
  async run(svcDeps) {
    ...
    const { HttpServer: httpServer, LiveModelServer: liveModel } = svcDeps;
    ...
    liveModel.model.on("broadcast", (...args) => this.broadcast(...args));
  }
  ...
  sendStateRefresh(clientId) {
    const applyOp = makeApplyOperation([
      ...
      {
        type: "$shallowAssign",
        value: this._svcDeps.LiveModelServer.model.getModelUnsafe()
      }
    ]);
