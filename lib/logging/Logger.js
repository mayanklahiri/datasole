...
const { get, isUndefined, map } = require("lodash");
...
class Logger {
  ...
  baseLog(logFields) {
    const logLevelInt = get(logLevels[logFields.level], "level");
    if (isUndefined(logLevelInt)) {
      throw new Error(`Unknown logging level "${logFields.level}"`);
    }
