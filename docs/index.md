...
features:
  ...
  - icon: ✓
    title: Correctness
    details: 122 unit tests + 13 e2e tests (Playwright, headless Chromium, production bundle). Coverage thresholds enforced on every push. Strict TypeScript with .d.ts on every export. Shared types between client and server — no codegen, no drift.
  - icon: 🛠
